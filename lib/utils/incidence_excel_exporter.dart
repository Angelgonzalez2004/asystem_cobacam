...
class IncidenceExcelExporter {
  ...
  static Future<String?> exportToExcel({
    ...
    List<String>? forceAllGroups, // Lista completa de grupos para generar hojas vacías
  }) async {
    ...
    final fileName = 'Incidencias_${specificGroupName ?? "General"}_${campus}_${DateFormat('yyyyMMdd_HHmm').format(DateTime.now())}.xlsx';
  ...
  static void _buildSheet(
    ...
    sheet.merge(CellIndex.indexByString("A1"), CellIndex.indexByString("U1"), customValue: TextCellValue(title));
    sheet.cell(CellIndex.indexByString("A1")).cellStyle = titleStyle;
    
    sheet.merge(CellIndex.indexByString("A2"), CellIndex.indexByString("U2"), customValue: TextCellValue("Plantel: $campus  |  Generado: ${DateFormat('dd/MM/yyyy HH:mm').format(DateTime.now())}"));
    sheet.cell(CellIndex.indexByString("A2")).cellStyle = CellStyle(horizontalAlign: HorizontalAlign.Center, fontSize: 10, italic: true);

    final headers = [
      'Fecha', 'Hora', 'Matrícula', 'Nombre del Alumno', 'Edad', 'Género', 'NSS',
      'Grupo', 'Ciclo', 
      'Nombre Tutor', 'Tel. Tutor', 'Tel. Alumno', 'Correo Inst.', 'Domicilio',
      'Alergias', 'Condiciones de Salud', 'Estado de Salud', // Columnas Médicas Separadas
      'Tipo de Falta', 'Observaciones',
      'Estatus', 'Resolución: Motivo', 'Resolución: Detalles', 'Resolución: Fecha' 
    ];

    int headerRow = 4;

    for (var i = 0; i < headers.length; i++) {
      var cell = sheet.cell(CellIndex.indexByColumnRow(columnIndex: i, rowIndex: headerRow - 1));
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      void write(int col, dynamic val) {
        ...
        cell.value = val is int ? IntCellValue(val) : TextCellValue(val?.toString() ?? 'N/A');
        
        // Estilo condicional para el estatus
        CellStyle style = CellStyle(
            backgroundColorHex: i % 2 == 0 ? ExcelColor.fromHexString("#F9FAFB") : ExcelColor.white,
      ...
      write(15, s?.healthConditions?.isNotEmpty == true ? s!.healthConditions : 'Ninguna');
      write(16, s?.generalHealthStatus?.isNotEmpty == true ? s!.generalHealthStatus : 'Sano');
      ...
      write(22, item.resolutionDate != null ? DateFormat('dd/MM/yyyy HH:mm').format(item.resolutionDate!) : '-');
    }
  }
}
