...
class _AttendanceQueryScreenState extends State<AttendanceQueryScreen> {
  ...
  Future<void> _loadGroupsAndStudents() async {
    try {
      ...
      final groupsRef =
          FirebaseDatabase.instance.ref('planteles/$_campus/groups');
      ...
      if (mounted) {
        setState(() {
          ...
          if (_selectedGroup != null &&
              !_dbGroups.any((g) => g.name == _selectedGroup)) {
            _selectedGroup = null;
          }
        });
      }
  ...
  Future<void> _fetchAttendanceData() async {
    ...
    try {
      ...
      if (_filterMode == FilterMode.day) {
        ...
        final cycleObj =
            _schoolCycles.firstWhere((c) => c.id == _selectedCycle);
      ...
      for (var i = 0; i < daysCount; i++) {
        ...
        final ref = FirebaseDatabase.instance
            .ref('planteles/$_campus/attendance/$_selectedCycle/$dateStr');
        ...
        if (snap.exists) {
          for (var child in snap.children) {
            ...
            final record = AttendanceRecord.fromFirebaseMap(
                child.key!, dateStr, data,
                campusId: _campus!, schoolCycle: _selectedCycle!);
  ...
  Future<void> _pickWeek() async {
    ...
    final picked = await showDatePicker(
      ...
      selectableDayPredicate:
          _isDaySelectable, // Corregido: Ahora respeta días no lectivos
      locale: const Locale('es', 'MX'),
    ...
    if (picked != null) {
      ...
      if (mounted) {
        UiHelpers.showSnackBar(context,
            'Semana seleccionada: ${DateFormat('dd/MM').format(validStart)} al ${DateFormat('dd/MM').format(validEnd)}');
      }
    }
  }
                      ...
                      onChanged: (val) {
                        if (val != null) {
                          ...
                          showDialog(
                              ...
                              builder: (ctx) => _buildLoadingOverlay());
              ...
              SingleChildScrollView(
                ...
                child: Row(
                  children: [
                    ...
                    _filterChip(
                        'Semana', FilterMode.week, Icons.date_range, _pickWeek),
                    const SizedBox(width: 8),
                ...
                children: [
                  Expanded(
                    child: TextField(
                      ...
                      decoration: _deco('Buscar Alumno...', Icons.search)
                          .copyWith(
              ...
              : filteredStudents.isEmpty
                  ? Center(
                      child: Column(
                          ...
                          const Text('No se encontraron alumnos')
                        ]))
                  : ListView.builder(
                      padding: const EdgeInsets.all(16),
                      ...
                      itemBuilder: (context, index) {
                        return _StudentAttendanceCard(
                          ...
                          stats:
                              _calculateStudentStats(filteredStudents[index]),
                          onTap: () =>
                              _showStudentDetail(filteredStudents[index]),
                        );
                      },
                    ),
        )
  ...
  Widget _filterChip(
      String label, FilterMode mode, IconData icon, VoidCallback onTap) {
  ...
  String _getRangeLabel() {
    if (_filterMode == FilterMode.day) {
      return DateFormat('EEEE dd/MM/yyyy', 'es_MX')
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          _infoGrid([
                            ...
                            _infoItem(
                                Icons.health_and_safety, 'NSS', s.nss ?? 'N/A'),
                            _infoItem(Icons.email_outlined, 'Correo',
                                s.institutionalEmail),
                            _infoItem(Icons.home_outlined, 'Domicilio',
                                s.placeOfResidence),
                            _infoItem(
                                Icons.calendar_today, 'Ciclo', s.schoolCycle),
                          ]),
                          const SizedBox(height: 24),
                          _sectionTitle('CONTACTO & TUTOR'),
                          _infoGrid([
                            _infoItem(Icons.family_restroom, 'Tutor',
                                s.guardianFullName),
                            _infoItem(
                                Icons.phone, 'Tel. Tutor', s.guardianPhone),
                            _infoItem(Icons.phone_android, 'Tel. Alumno',
                                s.studentPhone ?? 'N/A'),
                          ]),
                          const SizedBox(height: 24),
                          ...
                          _infoGrid([
                            ...
                            if (!s.isActive)
                              _infoItem(Icons.info_outline, 'Motivo Baja',
                                  s.deactivationReason ?? 'No especificado'),
                          ]),
                          const SizedBox(height: 24),
                          ...
                          Container(
                            ...
                            child: Column(children: [
                              _infoRowFull(Icons.favorite, 'Estado General:',
                                  s.generalHealthStatus ?? 'Sano'),
                              const SizedBox(height: 8),
                              _infoRowFull(Icons.warning_amber_rounded,
                                  'Alergias:', s.allergies ?? 'Ninguna'),
                              const SizedBox(height: 8),
                              _infoRowFull(
                                  Icons.medical_services_outlined,
                                  'Condiciones:',
                                  s.healthConditions ?? 'Ninguna'),
                              if (s.medicalAlert) ...[
                                const SizedBox(height: 8),
                                const Row(children: [
                            ...
                            itemBuilder: (ctx, i) {
                              ...
                              return ListTile(
                                ...
                                subtitle: rec != null && !isAbsent
                                    ? Text(
                                        'Entrada: ${rec.entryTime} - Salida: ${rec.exitTime ?? "-"}')
        ...
        builder: (context, setDialogState) => AlertDialog(
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                const Text(
                    'Configura los parámetros del reporte que deseas descargar.',
                ...
                SegmentedButton<FilterMode>(
                  segments: const [
                    ButtonSegment(
                        value: FilterMode.day,
                        label: Text('Día'),
                        icon: Icon(Icons.today)),
                    ButtonSegment(
                        value: FilterMode.week,
                        label: Text('Semana'),
                        icon: Icon(Icons.date_range)),
                    ButtonSegment(
                        value: FilterMode.cycle,
                        label: Text('Ciclo'),
                        icon: Icon(Icons.all_inclusive)),
                  ],
                  selected: {tempMode},
                  onSelectionChanged: (val) =>
                      setDialogState(() => tempMode = val.first),
                ),
                const SizedBox(height: 16),
                DropdownButtonFormField<String>(
                  value: exportScope,
                  ...
                  items: const [
                    DropdownMenuItem(
                        value: 'all', child: Text('Todo el Plantel')),
                    DropdownMenuItem(
                        value: 'group', child: Text('Por Grupo Específico')),
                    DropdownMenuItem(
                        value: 'student', child: Text('Alumno Específico')),
                  ],
                  onChanged: (val) {
                    setDialogState(() {
                      exportScope = val!;
                      ...
                      if (val == 'group') {
                        targetStudent = null;
                        tempGroup =
                            _dbGroups.isNotEmpty ? _dbGroups.first.name : null;
                      }
                      if (val == 'student') {
                        tempGroup = null;
                        targetStudent = null;
                ...
                if (exportScope == 'group')
                  DropdownButtonFormField<String>(
                    value: tempGroup,
                    decoration:
                        _deco('Seleccionar Grupo', Icons.groups_3_outlined),
                    items: _dbGroups
                        .map((g) => DropdownMenuItem(
                            value: g.name, child: Text('Grupo ${g.name}')))
                        .toList(),
                    ...
                    child: Column(
                      children: [
                        Autocomplete<Student>(
                          ...
                          optionsBuilder: (text) => _allStudentsMap.values
                              .where((s) =>
                                  s.fullName
                                      .toLowerCase()
                          ...
                          onSelected: (s) =>
                              setDialogState(() => targetStudent = s),
                          fieldViewBuilder: (ctx, ctrl, node, onSub) =>
                              TextField(
                            controller: ctrl,
                            focusNode: node,
                            decoration:
                                _deco('Nombre o Matrícula', Icons.search)
                                    .copyWith(
                                        suffixIcon: targetStudent != null
                        ...
                        if (targetStudent != null)
                          Padding(
                            ...
                            child: Chip(
                              ...
                              label: Text(targetStudent!.fullName,
                                  style: const TextStyle(
                                      fontSize: 11,
                                      fontWeight: FontWeight.bold)),
                              onDeleted: () =>
                                  setDialogState(() => targetStudent = null),
                            ),
                          ),
                      ],
                    ),
            ...
            ElevatedButton.icon(
              ...
              onPressed: () {
                ...
                if (exportScope == 'student') {
                  ...
                  exportList = _allStudentsMap.values
                      .where((s) => s.group == tempGroup)
                      .toList();
                } else {
  ...
  void _showExportConfirmation(List<Student> students,
  ...
  Future<void> _processExport(
    ...
    showDialog(
      context: context,
      barrierDismissible: false,
      builder: (context) => _buildLoadingOverlay(message: 'Generando Reporte Avanzado...', subMessage: 'Calculando estadísticas y organizando datos.'),
    );
    ...
    for (var s in students) {
      computedStats[s.studentId] = _calculateStudentStatsForExport(s,
          cycle: targetCycle!,
          mode: targetMode,
          date: targetDate,
          week: targetWeek);
    ...
    try {
      await AttendanceExcelExporter.exportAdvancedReport(
        ...
        rangeLabel: _getRangeLabelForExport(
            targetMode, targetDate, targetWeek, targetCycle!),
        cycle: targetCycle,
  ...
  Widget _buildLoadingOverlay({String message = 'Consultando datos...', String subMessage = 'Espere unos segundos por favor.\nEstamos organizando la información.'}) {
    return Dialog(
      backgroundColor: Colors.transparent,
      elevation: 0,
      child: Container(
...
class _StudentAttendanceCard extends StatelessWidget {
  ...
  const _StudentAttendanceCard(
      {required this.student, required this.stats, required this.onTap});
  ...
  Widget build(BuildContext context) {
    ...
    return Card(
      ...
      child: ListTile(
        ...
        subtitle: Row(
          children: [
            Text('${student.group} • ${student.studentId}',
