  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
                  ...
                  children: [
                    if (_campusId != null)
                      Padding(
                        ...
                        child: Text('PLANTEL $_campusId', style: TextStyle(color: theme.colorScheme.primary, fontWeight: FontWeight.bold, letterSpacing: 1.2)),
                      ),
                        ...
                        child: Padding(
                          ...
                          child: DropdownButtonHideUnderline(
                            child: DropdownButton<SchoolCycle>(
                              ...
                              items: _schoolCycles.map((cycle) => DropdownMenuItem(value: cycle, child: Text('Ciclo Escolar: ${cycle.id} (${cycle.type})', style: const TextStyle(fontWeight: FontWeight.bold)))).toList(),
                            ),
                          ),
                        ),
                      ),
                      ...
                      child: _groups.isEmpty
                          ? Center(
                              child: Column(
                                ...
                                children: [
                                  Icon(Icons.class_outlined, size: 64, color: Colors.grey.withValues(alpha: 0.3)),
                                  const SizedBox(height: 16),
                                  Text("No hay grupos registrados", style: TextStyle(color: Colors.grey.shade500, fontSize: 16)),
                                ],
                              ),
                            )
                          : GridView.builder(
                              ...
                              itemBuilder: (context, index) {
                                ...
                                switch (group.semester) {
                                  case 1: semesterColor = Colors.blue.shade100; break;
                                  case 2: semesterColor = Colors.indigo.shade100; break;
                                  case 3: semesterColor = Colors.green.shade100; break;
                                  case 4: semesterColor = Colors.teal.shade100; break;
                                  case 5: semesterColor = Colors.orange.shade100; break;
                                  case 6: semesterColor = Colors.deepOrange.shade100; break;
                                  default: semesterColor = Colors.grey.shade200;
                                }
                                Color textColor = HSLColor.fromColor(semesterColor).withLightness(0.3).toColor();

                                return FadeInUp(
                                  delay: Duration(milliseconds: 30 * index),
                                  child: Card(
                                    elevation: 0,
                                    shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20), side: BorderSide(color: semesterColor, width: 1)),
                                    ...
                                    child: InkWell(
                                      ...
                                      child: Stack(
                                        children: [
                                          ...
                                          Positioned(
                                            right: -20, top: -20,
                                            child: CircleAvatar(radius: 50, backgroundColor: semesterColor.withOpacity(0.3)),
                                          ),
                                          Padding(
                                              ...
                                              children: [
                                                Row(
                                                  ...
                                                  children: [
                                                    Container(
                                                      ...
                                                      decoration: BoxDecoration(color: semesterColor, borderRadius: BorderRadius.circular(8)),
                                                      child: Text('${group.semester}º Semestre', style: TextStyle(color: textColor, fontWeight: FontWeight.bold, fontSize: 12)),
                                                    ),
                                                ...
                                                Row(
                                                  children: [
                                                    ...
                                                    Text('${group.studentCount} Alumnos', style: TextStyle(color: Colors.grey.shade600, fontSize: 13)),
                                                  ],
                                                ),
                                              ],
                                            ),
                                          ...
                                          Positioned(
                                            bottom: 8, right: 8,
                                            child: IconButton(
                                              icon: Icon(Icons.delete_outline, size: 20, color: theme.colorScheme.error.withOpacity(0.6)),
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        label: const Text('Nuevos Grupos', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
      ),
    );
  }
            ...
            children: [
              TextField(
                ...
                decoration: InputDecoration(
                  labelText: isEditing ? 'Identificador' : 'Identificadores (separados por coma)',
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () {
                ...
                // Procesar entrada
                final List<String> inputs = isEditing ? [input] : input.split(',').map((e) => e.trim()).where((e) => e.isNotEmpty).toList();
                ...
                for (final groupIdentifier in inputs) {
                    ...
                    final int semester = int.tryParse(groupIdentifier.substring(0, 1)) ?? 0;
                    ...
                    if (!isValid) {
                      firstError ??= 'Grupo $groupIdentifier no válido para ciclo $cycleType';
                      continue;
                    }
                ...
                if (createdCount > 0) {
                    UiHelpers.showSnackBar(context, isEditing ? 'Grupo actualizado.' : '$createdCount grupos creados exitosamente.');
